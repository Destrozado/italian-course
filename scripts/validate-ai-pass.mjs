...
//
// Pase de validación de quórum multi-proveedor (cross-vendor) con AUTO-FALLBACK.
// Complementa al skill `gsd-validate-exercise` (que corre los pases Claude vía Task).
// Aquí UN pase lo emite un modelo externo (Gemini / DeepSeek) sobre UN ejercicio —
// NUNCA batched (VAL-03): cada invocación ve un solo ejercicio.
//
// Zero-deps: sólo `https`/`fs` nativos. Lee claves de `.env`
// (GEMINI_API_KEY, DEEPSEEK_API_KEY).
//
// AUTO-FALLBACK: si el modelo primario agota su límite (429) tras reintentos,
// se prueba con el siguiente modelo de `--fallback` automáticamente. El `by`
// registrado es SIEMPRE el modelo que de verdad respondió — el quórum se computa
// sobre el `by` real, así que el fallback no rompe la distinción >=2 by.
//
// Uso:
//   node scripts/validate-ai-pass.mjs <exercise-id> [opciones]
//     --model=<id>        modelo primario (default gemini-2.5-flash)
...
//     --write             inserta el pase en passes[] y re-deriva status
...
//
// El pase Claude (claude-opus-4-7 / claude-sonnet-4-6) NO se hace aquí: corre vía
// el skill gsd-validate-exercise (Task subagent). Ver docs/VALIDACION-QUORUM.md.
...
const PRIMARY = getOpt('model', 'gemini-2.5-flash');
...
function providerFor(model) {
  ...
  throw new Error(`modelo no soportado por este script: ${model} (usa gemini-* o deepseek-*; Claude va por el skill)`);
}
...
function findExercise(id) {
  for (const dir of ['content/exercises', 'tests/fixtures']) {
