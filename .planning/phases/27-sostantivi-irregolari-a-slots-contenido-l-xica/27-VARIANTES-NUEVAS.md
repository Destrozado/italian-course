...
|-------------|--------------|-----------------|--------|---------|--------------|----------------------------------------------------|
| `tmp-sost-acc-virtu` | `sustantivos-irregulares-invariabili-accentate` | virtu / vocal tonica final INVARIABLE | `Una virtù, due ___.` | `["virtù", "virtù", "virtùe", "virti"]` | 0 | **VERIFICACION:** virtù (virtud) acaba en vocal tonica final acentuada (ù grave) -> INVARIABLE: `la virtù` / `le virtù` identicas. El calco español `*virtudi`/`*virtudes` es erroneo. R1: el prompt fija el singular y pide el plural (mecanica del slot, no leak). R5: ATENCION — la opcion 1 duplica la respuesta; **CORREGIR en materializacion** (ver nota). Distractoras intentadas: `virtudi`=calco español flexionado (la trampa), `virtùe`=flexion inventada, `virti`=truncamiento sin tilde. Respuesta = la MISMA forma `virtù` (con tilde grave). Palabra NUEVA. **OJO TILDE: ù grave obligatoria.** |
...
| `tmp-sost-acc-eta` | `sustantivos-irregulares-invariabili-accentate` | eta / vocal tonica final INVARIABLE (-tà) | `Una età, due ___.` | `["età", "etadi", "ete", "etàe"]` | 0 | **VERIFICACION:** età (edad) acaba en vocal tonica final acentuada (à grave, familia -tà) -> INVARIABLE: `l'età` / `le età` identicas. R1: sin leak. R5: `etadi`=calco flexionado (la trampa), `ete`=flexion inventada, `etàe`=flexion inventada con tilde. 4 distintas. Respuesta = la MISMA forma `età` (con tilde grave). Palabra NUEVA. **OJO TILDE: à grave obligatoria.** (Nota: el prompt usa `Una età` por paralelismo con el slot; en italiano real seria `un'età`, pero el slot fija la mecanica `Una/Un X, due ___` — el quorum confirmara si conviene `Un'età`; ver nota de materializacion.) |
