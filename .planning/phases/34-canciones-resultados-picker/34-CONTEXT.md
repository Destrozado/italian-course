...
### Escala tipográfica y rejilla de espaciado — LOCKED (heredadas, fidelidad al handoff)
- **D-15 (escala tipográfica):** La **escala de tamaños y pesos de fuente es verbatim del `design_handoff_italiano_redesign/README.md` §Tipografía** y hereda el sistema de 3 familias (Spectral / Hanken Grotesk / Space Grotesk) + tokens `--ed-*` establecido en Phase 32. Esto es **brownfield de fidelidad alta**: la regla genérica de "máx. 4 tamaños / 2 pesos" (pensada para diseños greenfield) **NO aplica** — reducir la escala sería desviarse del handoff, que es la fuente de verdad declarada en `<canonical_refs>`. Los ~12 roles de tamaño y los pesos 400–800 son la realidad del design system heredado, no valores inventados.
- **D-16 (rejilla de espaciado):** Las **excepciones de espaciado 22px (`--ed-space-screen`), 46px (tile de lista), 66px (portada "Continuar") y 54px (columna fija FALLÓ/N ej.) son verbatim del handoff §Espaciado** y vienen heredadas de Phase 32 (`--ed-space-screen: 22px` ya está en `main`). La regla genérica de "todo espaciado múltiplo de 4" **NO aplica** a estos valores heredados/del handoff. El resto de la escala sí es 4-point (4/8/16/24/32/48/64).
