...
provides:
  ...
  - "tests/content-fare-indefiniti.test.js — 13 describe / 91 tests de invariantes permanentes, con 5 desviaciones declaradas y 65 pruebas negativas ejecutadas (12 de autoria + 3 de CR-02 + 4 de WR-01 + 46 post-quorum)"
...
patterns-established:
  ...
  - "BORRAR ES LA UNICA CORRECCION QUE NO PUEDE FALLAR. Si una frase necesita cuatro rondas de precision y cada intento introduce una falsedad distinta, no tiene forma verdadera al nivel de generalidad que intenta: se retira entera. Y el criterio que decide no es si la frase se puede hacer cierta, sino si DISCRIMINA algo — una frase verdadera que no elimina ninguna opcion del ejercicio es superficie de error a cambio de cero. Aqui el predicado no descartaba ninguna de las 4 options porque las 4 ya eran no personales por el gate del POOL, asi que ni acotada valia la pena."
  - "UNA SUPRESION NO DEJA RASTRO EN EL CONTENIDO —ese es el punto—, asi que su motivo tiene que vivir en el `notes` Y estar gateado. Sin eso, el proximo lector repone la frase para redondear el parrafo y el ciclo empieza otra vez. Y hay que gatear tambien lo que la supresion no puede llevarse por delante: aqui, la acotacion de persona de la frase que queda, sin la cual el texto reafirma sin querer lo que se acaba de borrar."
...
- **Suite:** 912 pass / 0 fail (baseline) -> **995 pass / 0 fail**; **1026** tras CR-02, WR-01, las siete rondas de quorum y los fixes de 43-01
...
**Post-review:** `20a5cc6` (fix) — CR-02, `6495443` (fix) — WR-01, y `dc2450a`, `b83f11a`, `90e0c3a`, `b1e101e`, `9d4e1a4`, `be0acb3` y `adef09d` (fix) — las siete rondas de quorum. Ver sus secciones propias mas abajo.
...
- `tests/content-fare-indefiniti.test.js` — **creado**, 1.939 lineas al cierre, 13 `describe` / 91 tests. Las CINCO desviaciones respecto del analogo declaradas en la cabecera, mas el COROLARIO de frontera de palabra de CR-02 y los gates de WR-01 y de la premisa corregida del gerundio passato.
...
### Septima ronda — borrar, no acotar. Y la segunda vez que un gate mio exigio el defecto

Los dos modelos coinciden y el hallazgo es correcto: la frase de cierre **se borra**. Supresion pura en `adef09d` — la explanation pasa de **806 a 627** caracteres y lo nuevo es **prefijo estricto** de lo viejo, ni una palabra en su lugar.

**Verifique las dos razones, y la segunda es la que decide:**

1. **Falsa** al nivel de generalidad que intentaba. La orden negativa usa infinitivo **solo en la 2a del singular**; con `Lei`, `voi` y `noi` conserva forma **conjugada** (`non faccia`, `non fate`, `non facciamo`) — que es exactamente el paradigma de cinco casillas que enseña la categoria hermana de esta misma fase. El parrafo habia acotado bien la persona dos frases antes y el resumen perdia la acotacion.
2. **Vacia**, y esto sobrevive a cualquier acotacion. Lo comprobe sobre el fichero: las **4 options de cada una de las 3 variantes son formas no personales** por el gate del POOL (D-43-13), asi que el predicado «lo que va es una forma no personal y no una conjugada» eliminaba **0 de 4**. Ni acotada a la 2a del singular aportaria nada: frase verdadera y vacia, con superficie de error a cambio de cero.

Sonnet proponia añadir «de tú» y habria sido cierto; no lo hice, porque el argumento de la vacuidad lo hace irrelevante. El parrafo **termina seco a proposito**.

**La poda, y es la segunda vez que me pasa lo mismo.** El gate `P1 en POSITIVO` que añadi en la 6a ronda obligaba a que la explanation cerrara con una enumeracion **completa** de disparadores. Lo escribi para impedir que la lista se quedara corta — y la respuesta correcta resulto ser que **la lista no debia existir**. Retirado con su explicacion. Es la misma leccion que la poda de la 4a ronda: **un gate de presencia sobre prosa congela la forma que el texto tenia el dia que se escribio, y si esa forma era el defecto, el gate lo perpetua.** Los gates que miran el contenido (POOL, options, concordancia) no tienen ese problema, y son los que sobreviven a las siete rondas sin tocarse.

**Gates.** Comprobe primero que el gate tematico ampliado de la 6a ronda **no** cubria esta frase —no lleva ninguna de sus dos familias—, asi que hacia falta regresion propia; se añade a la lista por slot, junto con **la version acotada que Sonnet proponia**, que sigue siendo vacia. Y las mutaciones descubrieron **dos huecos mas**:

- **Quitar la acotacion de persona de la frase que QUEDA** dejaba la suite verde. Sin ella, la explanation reafirma sin querer justo lo que se acaba de borrar: el defecto se habria mudado de frase.
- **Vaciar el `notes` del porque del borrado** tambien pasaba limpio. Una supresion **no deja rastro en el contenido** —ese es el punto—, asi que si el motivo no esta gateado, dentro de seis meses alguien repone la frase para redondear el parrafo y el ciclo de cuatro rondas empieza otra vez.
...
- **21 gates nuevos** entre las siete rondas (y **2 podados con nota**, los dos por exigir prosa que despues hubo que borrar), cada uno con mensaje que nombra el hallazgo, y **46 mutaciones** verificadas con restauracion byte a byte. Primera ronda (7): reintroducir `facendo` (fail 2), quitar el calco (fail 1), volver a afirmar que el encaje exige el compuesto (fail 1), volver a la errata `a tú` (fail 1), borrar el override del `notes` (fail 1), borrar la premisa corregida (fail 1), quitar el compuesto del marco progresivo (fail 2). Segunda ronda (6): volver a `de ellas` (fail 1), volver a la prohibicion desnuda (fail 2), quitar los dos introductores correctos (fail 2), quitar la justificacion de la key concesiva (fail 1), un absoluto desnudo en otra explanation cualquiera (fail 1), y borrar la condicion del `notes` (fail 1). Tercera ronda (6): concordar el participio distractor (fail 1), reponer la taxonomia (fail 1), reponer la regla del introductor (fail 1), reponer la comparacion con el espanol (fail 1), hacer crecer el texto por encima del tope (fail 1), y reponer la etiqueta `implicita-causal` en la tabla (fail 1). Cuarta ronda (8): los dos absolutos del recorte (fail 2 y fail 1), los dos del infinito presente (fail 1 y fail 1), el tope por slot (fail 1), y **tres que recortan DE MAS** para verificar el riesgo simetrico: stub en cada slot (fail 2 y fail 2) y perdida del desarrollo del imperativo negativo (fail 1). Quinta ronda (6): la frase retirada vuelve literal (fail 2), tres formulaciones distintas del mismo TEMA incluida una en otro slot (fail 1 cada una), y las dos mitades del par pedagogico borradas por separado (fail 1 cada una) — estas dos ultimas son el hueco propio que se encontro y cerro. Sexta ronda (8): la frase retirada vuelve literal (fail 2), tres superficies distintas de la familia 2 del gate (fail 1 cada una, una de ellas descubriendo que la comparacion era sensible a mayusculas), y cuatro contra la completitud de la enumeracion — caer cada disparador por separado y partirla en dos frases (fail 1 cada una). Septima ronda (5): la frase borrada vuelve literal (fail 1), vuelve en su version acotada que sigue siendo vacia (fail 1), y tres sobre lo que la supresion NO puede llevarse por delante — la acotacion de persona de la frase que queda, y las dos mitades del porque en el notes (fail 1 cada una).
- Prompts y keys **sin tocar en ninguna de las siete rondas**: las 18 variantes quedan byte a byte identicas. Solo cambian 3 listas de `options` (1a ronda), 4 explanations y el `notes`. `src/` byte-intacto, counts sin tocar. Suite **1026 pass / 0 fail**, 91 tests en el fichero de la categoria.
