...
- 🚧 **v2.0 — Paradigma completo de `fare` (4 categorías por modo)** — Phases 40-45 (ACTIVE, iniciado 2026-07-28). El paradigma entero del verbo más frecuente e irregular con un ejercicio por casilla: `fare-indicativo` (8 slots) · `fare-congiuntivo` (4) · `fare-cond-imperativo` (3) · `fare-indefiniti` (6) = **22 slots / 113 variantes** autoradas por quórum cross-vendor R1-R7 en slot+variantes, + migración `12→13` con reset selectivo + integración lockstep. (El título dice 4 slots de congiuntivo y la estimación de plan-time era de 21 slots y ≈107 variantes; el real es 5 slots y 22/113 porque CONG-04 pidió un slot para el DISPARADOR — ver la nota D-42-01 de Phase 42.) Cierra con **Phase 45 — Deuda del arnés de tests**, añadida el 2026-08-12 a raíz de la propia auditoría del milestone: paga los tres sitios donde el arnés no vigilaba lo que su prosa decía vigilar (`tests/fixtures/` fuera de la suite canónica, el 3er array de conteo fuera del gate anti-ceguera, y la cabecera del reporter cuatro milestones desfasada). 26/26 requirements (6 IND + 4 CONG + 3 CI + 4 INDEF + 2 MIG + 4 INT + 3 DEUDA). **Brownfield PURO DE CONTENIDO + arnés: motor v1.4 NO tocado (cascada D-54 con EXACTAMENTE 2 call-sites).** Numeración CONTINÚA desde Phase 39.
...
- [x] **Phase 45: Deuda del arnés de tests** — los tres sitios donde el arnés no vigila lo que dice vigilar: `tests/fixtures/` fuera de la suite canónica, el 3er array de conteo fuera del gate anti-ceguera, y la cabecera del reporter cuatro milestones desfasada. (completed 2026-08-13)
...
- [x] 41-02-PLAN.md — Los 4 tiempos COMPUESTOS: passato prossimo, trapassato prossimo, futuro anteriore y trapassato remoto (24 variantes), más `tests/content-fare-indicativo.test.js` con los 8 gates de la categoría como invariantes permanentes
...
- [x] 42-01-PLAN.md — Alta de la categoría (`categories.json` order 16 + línea del smoke) y los 2 tiempos SIMPLES: congiuntivo presente e imperfetto, 12 variantes, tracer sobre el presente verificado antes de expandir
...
**Goal**: El milestone cierra con las 4 categorías registradas y los conteos re-sincronizados en lockstep — 4 entradas nuevas en `categories.json` (order 15-18, `origen: "ia-quorum"`), los **2** arrays de count que quedaron ciegos (`CATEGORIES` del reporter y `REAL_CATEGORIES` del back-compat) más el **gate anti-ceguera** que hace estructuralmente imposible la próxima ceguera —, más los cruces multi-categoría de `fare` y el gate que demuestra que el motor v1.4 sigue byte-intacto. `TOTAL_EXPECTED` y la fórmula del baseline-guard ya eran `reduce` sobre el array, así que se re-suman solos y NO se editan; el smoke paramétrico y `CATEGORIES_WITH_EXPLANATIONS` los fueron ampliando las propias Phases 41/42/43. Transversal y de bajo riesgo (metadata + counts, cero motor); mirror de Phase 31 (v1.7) y Phase 39 (v1.9). El registro operativo de cada categoría puede ocurrir en su fase de contenido; INT-01 se cierra y verifica aquí.
...
**Success Criteria** (what must be TRUE):
  ...
  2. Los **2** arrays de conteo que quedaron CIEGOS (`CATEGORIES` de `scripts/run-validation-271.mjs` y `REAL_CATEGORIES` de `tests/fixtures/slot-variants-integration.test.js`) ganan +4 entradas con `expected` DINÁMICO leído del JSON (`slotCountOf` / `.exercises.length`, nunca número mágico) y `TOTAL_EXPECTED` se re-suma solo: **225 → 247 en disco** tras Phase 43 (**22 slots nuevos**: 8 indicativo + 5 congiuntivo + 3 cond-imperativo + 6 indefiniti), y **250** con los 3 cruces del plan 44-02. El tercer array (`CATEGORIES_WITH_EXPLANATIONS`) y el smoke paramétrico los fueron ampliando las Phases 41/42/43, y la fórmula del `TOTAL_EXPECTED_BASELINE` ya es un `reduce` sobre el array: ninguno de los dos se edita. Y como el item era el más fácil de olvidar —se olvidó **tres fases seguidas**, con el reporter emitiendo `225/225 PASS` ciego a los 22 slots—, la fase entrega además el **gate anti-ceguera** (`tests/count-arrays-lockstep.test.js`): pone rojo cualquier categoría registrada en `categories.json` y no enganchada, con golden-negative committeado de la colisión de prefijo `fare-ind`.
...
>
> **Cierre efectivo (2026-08-11).** La pasada TOP-LEVEL corrió y el estado `FAIL (247/250)` descrito arriba **ya no es el vigente**: los 3 cruces están `validated` (Opus + Sonnet, 2 pases de `by` distintos cada uno, cero `disputed`) y el marcador cierra en `VAL-06 (250/250 validated): PASS` con `Milestone gate PASS`, `VAL_07_STRICT` en 1081/0 y la suite en 1063/0. INT-03 e INT-04 pasan a `Complete`. Hizo falta **6 rondas para 3 ejercicios**: `fare-indicativo-301` cayó `incorrecta` en la 1ª (C5-leak en los 3 glosses ES + C4 en la explanation) y `fare-indefiniti-300` cayó dos veces por C2, porque **G3 comprobaba que el marcador del complemento ESTUVIERA, no que EXCLUYERA** — un gate verde sobre un ejercicio con dos respuestas defendibles, cazado por Sonnet y no por el test.
>
> **UAT (2026-08-11) y cierre de gaps.** Los tests 1 y 2 pasaron (18 categorías en su orden; cascada D-54 verificada jugando de verdad sobre los 3 cruces). El test 3 —la decisión sobre los 9 warnings + 3 info del code review— se cerró como `issue`: el autor decide **arreglar WR-01, WR-02 y WR-03** antes de `/gsd-complete-milestone v2.0` (planes 44-03 y 44-04, **fixes SOLO de test, cero cambio de contenido**) y **aceptar WR-04…WR-09 e IN-01…IN-03 como deuda documentada** en `44-REVIEW.md`. Los tres elegidos son los que tocan el Core Value: WR-01 es el arnés quedándose verde ante el mismo patrón que ya mordió en CR-01, y WR-03 es el único sitio donde el quórum cazó un leak C5 real (`fare-indicativo-301`) sin que nada mecánico proteja hoy contra su regresión.
