...
[resuelto — GAP-01 cerrado en plan 32-03 eliminando Pico]
...
result: PASS (tras 32-03) — Primer intento (remap de --pico-*) FALLÓ: la card blanca de Pico seguía encima (Pico define sus vars bajo :root:not([data-theme=dark]), gana especificidad). Resuelto eliminando Pico (decisión del autor). Verificado en navegador (headless): Home sobre papel sin card blanca, títulos Spectral, sin azul Pico.
...
result: PASS (tras 32-03) — tricolore visible en la cabecera de la Home, intacto tras eliminar Pico. Verificado en captura headless.
...
result: PASS (tras 32-03) — dots y barras de racha renderizan con el color del estado; intactos tras eliminar Pico (en datos vírgenes todos no-hecha=gris; el mapeo currentColor desde badge-* D-01 se conserva). Verificado en captura headless.
...
status: resolved
resolution: Cerrado en plan 32-03. El remap de --pico-* no ganó la cascada (Pico define sus vars bajo :root:not([data-theme=dark])); por decisión del autor se ELIMINÓ Pico y app.css aporta ahora el reset/base de elementos con la paleta Editoriale. Verificado en navegador (Home/Backup/Picker/Session legibles, sin card blanca). Invierte la decisión bloqueada D-03.
requirement: FND-03
