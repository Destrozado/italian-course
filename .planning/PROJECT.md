...
- Cobertura de regla por slots: `-ARE` · `-ERE` · `-IRE` simple (dormire/partire/aprire) · `-IRE` con `-isc-` (finire/capire/preferire — la sub-regla trampa) · ortográficos `-care/-gare` (giochi/paghi) y `-ciare/-giare` (cominci/mangi).
- Tipos multi-choice + word-buttons + match (match solo donde el pareo NO sea derivable por raíz — DESIGN RULE Phase 4).
- Variantes nuevas autoradas por quórum cross-vendor R1-R7 (patrón v1.4+, skill `gsd-validate-exercise`).
- Integración lockstep: migración `schemaVersion 9→10` con reset selectivo SOLO de la categoría nueva, bump de `TOTAL_EXPECTED`, +1 entrada en el smoke paramétrico.

**Key context:** Brownfield puro — el motor v1.4 (cascada D-54, sampler, slot-engine) NO se toca; solo contenido + migración + counts, igual que cada categoría de v1.5/v1.6. Análogo de referencia: v1.2 (alta de Articoli/Partitivi) + el patrón slot de v1.6. Posible cruce multi-cat con avere/essere (passato prossimo) a decidir en requirements.

**v1.6 — Conversión a slots: categorías restantes (CONV-01 cierre) — SHIPPED 2026-06-09.** Las 6 categorías legacy restantes convertidas al modelo slot+variantes + migración `8→9`. CONV-01 CERRADO: las 9 categorías de gramática en formato slot+variantes unificado. Detalles en `.planning/milestones/v1.6-ROADMAP.md` y en §Current State.

**Scope boundary:** El bloque Canciones sigue como modo standalone aparte (no aplica slot+variantes). Otros tiempos verbales (Imperfetto / Futuro / Condizionale / Congiuntivo — TENSE-X1..X4), AUTHOR-01, CATPROC, MUSIC-X1 y responsive móvil siguen en backlog para milestones futuros.
...
<!-- Milestone v1.7 activo (iniciado 2026-06-16). Requisitos detallados con REQ-IDs en .planning/REQUIREMENTS.md; trazabilidad a fases en .planning/ROADMAP.md. -->

**Milestone v1.7 — Presente regolare (10ª categoría):** alta de la categoría `presente-regolare` en formato slot+variantes (slots -are/-ere/-ire/-isc-/ortográficos), variantes por quórum cross-vendor R1-R7, integración lockstep (migración `9→10` + counts + smoke). Brownfield puro: motor v1.4 intacto. Ver `REQUIREMENTS.md`.
