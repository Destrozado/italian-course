...
## Current Milestone: v1.6 — Conversión a slots: categorías restantes (CONV-01 cierre)

**Goal:** Convertir las 6 categorías que aún quedan en formato legacy `payload` (verbos + morfología/léxico) al modelo slot+variantes, una fase por categoría, dejando las 9 categorías en un único formato unificado y cerrando CONV-01.

**Target features:**
- **Migración `schemaVersion 8→9`** (`migrate8to9`/`hydrateV9`): reset selectivo de progreso de las 6 categorías a convertir (predicado de 6 prefijos, patrón Phase 18); las 3 ya convertidas (preposiciones, articoli, partitivos) conservan progreso byte-intacto; `backup.js` round-trip v9. Va PRIMERA: bloquea la renumeración de ids del contenido.
- **6 conversiones, 1 fase por categoría** replicando el patrón de Phases 19/20: **Avere**, **Essere**, **Verbi di movimento** (verbos) + **Genere e numero**, **Professioni**, **Sostantivi irregolari** (morfología/léxico). Cada una: reagrupar a slots por regla con explanation a nivel de slot → autorar variantes nuevas por quórum cross-vendor R1-R7 (+ slots de huecos) → sincronizar counts derivados del JSON.
- **Validator + smoke paramétrico** (ya bifurcado por shape desde v1.4) verdes en cada conversión; sin tocar motor/sampler/cascada/loader.

**Open question (a resolver en discuss/plan de cada léxica):** para las categorías léxicas puras (Professioni, Sostantivi irregolari) evaluar si el modelo slot+variantes aporta valor real o si conviene dejarlas como slots-de-1 reagrupados — no toda categoría tiene "regla con variantes intercambiables".

**Last shipped:** v1.5 — Conversión a slots: Bloque Artículos (2026-06-05). Articoli (56→34 slots) + Partitivi (44→19 slots) + migración `7→8`. 9/9 requirements, 358/358 tests. 3/9 categorías ya en formato slot+variantes.

**Scope boundary:** v1.6 cierra CONV-01 (las 9 categorías de gramática unificadas). El bloque Canciones queda como modo standalone aparte (no aplica slot+variantes). Categorías de tiempos verbales futuras (TENSE-X1..X4) y bridges multi-categoría siguen en backlog.
...
<!-- Milestone v1.6 — Conversión a slots: categorías restantes (CONV-01 cierre). REQ-IDs en REQUIREMENTS.md. -->

**Milestone v1.6 — Convertir las 6 categorías restantes a slot+variantes (cierre de CONV-01):**
- Migración `schemaVersion 8→9` con reset selectivo de las 6 categorías a convertir (las 3 ya convertidas conservan progreso); backup v9
- Avere / Essere / Verbi di movimento (verbos) reagrupados a slots por regla + variantes nuevas por quórum
- Genere e numero / Professioni / Sostantivi irregolari (morfología/léxico) reagrupados a slots por regla + variantes nuevas por quórum — evaluando por categoría si slot+variantes aporta valor o si quedan como slots-de-1 reagrupados
- Validator + smoke paramétrico verdes en cada conversión; counts derivados del JSON; explanations a nivel de slot; sin tocar el motor
...
**Last activity:** 2026-06-08 — Phase 26 (Professioni → slots, híbrida) completada; v1.6 6/7 fases (pendiente solo Phase 27 Sostantivi irregolari).
