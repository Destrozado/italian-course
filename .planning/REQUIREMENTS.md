...
- [x] **ART-02**: Se autoran variantes nuevas (patrón D-85: Claude propone → autor revisa → quórum cross-vendor R1-R7) en los slots de Articoli cuya regla admite reformulación; cada variante nueva pasa el quórum antes de entrar. Los huecos de regla detectados durante la autoría se añaden como slots nuevos.
...
- [x] **ART-04**: La estructura final de Articoli pasa el validator y el smoke paramétrico (con los hardcodes de count re-sincronizados al nº real de slots, como en D-17-04), con la cobertura de explanations a nivel de slot preservada.
...
- [x] **PART-03**: La estructura final de Partitivi pasa el validator y el smoke paramétrico (counts re-sincronizados), con la cobertura de explanations a nivel de slot preservada.
