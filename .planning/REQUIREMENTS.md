...
- [x] **VAL-04**: Cada ejercicio recibe ≥2 pases de AIs distintos para alcanzar `status: validated`. La AI que valida se registra explícitamente en `passes[].by` (ej. `claude-opus-4-7@2026-05-25`, `gemini-pro@2026-05-25`). Un solo pase deja `status: pending`. Cualquier verdict `incorrecta` cambia `status: disputed`.
...
- [x] **VAL-06**: Cobertura final: 271/271 ejercicios con `validation.status === "validated"` antes de cerrar Phase 10 (o `disputed` resuelto manualmente por el autor → re-validado → validated).
