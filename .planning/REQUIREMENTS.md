...
- [ ] **VAL-01**: El schema de ejercicios soporta un campo opcional `validation` con estructura `{status: "pending"|"validated"|"disputed", passes: [{by, date, verdict, concerns?}]}`. Schema validator acepta ejercicios SIN el campo (backward-compat con los 271 actuales).
