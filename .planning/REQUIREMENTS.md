...
- [ ] **SEED-03**: Añadir Essere como 7ª categoría dedicada con cobertura A1 mínima — conjugación presente (6 personas) + identidad + nacionalidad + profesión (contraste avere) + estado/condición + cópula clasificatoria + participio passato prossimo (`stato/stata/stati/state`). ≥30 ejercicios base + ≥1 cruce multi-categoría que ejercita la cascada D-54. Detectado durante UAT post-Phase 4 — la simetría con Avere es fundamental para A1 (el plan v1.0 olvidó essere como categoría independiente). Scope: Phase 5.
